export const IPC_CHANNELS = {
  ...
  /** Main → renderer: trigger `ArchonContributionRegistry.invokeCommand` (e.g. dev menu). */
  ...
  /** Local marketplace catalog (`marketplace-index.json` under dist/plugins or ARCHON_MARKETPLACE_DIR). */
  MARKETPLACE_LIST_PLUGINS: "plugin:marketplace-list",
  /** Install a `.archonplugin` from the marketplace directory by basename (safe join). */
  ...
  PLUGIN_LOAD_ARCHON_FROM_PARENT: "plugin:load-archon-from-parent",
  ...
  PLUGIN_MAINT_FORMAT_ARCHON: "plugin:maint:format-archon",
  ...
  ARCHON_CLIENT_LOG: "archon:client-log",
  ...
  ARCHON_UNDO: "archon:undo",
  ARCHON_REDO: "archon:redo",
  ...
  /** Renderer → main: read current `archon-workspace.json` slot files for RxDB import. */
  WORKSPACE_RXDB_MIRROR_PULL: "workspace:rxdb-mirror-pull",
  /** Renderer → main: write mirror payload when `ARCHON_WORKSPACE_RXDB_AUTHORITY=1` (ADR-016 Phase 4). */
