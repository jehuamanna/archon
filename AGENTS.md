...
Archon is a programmable knowledge system distributed as an Electron desktop app plus a web app, backed by a Fastify sync API against Postgres and exposed to AI agents via an MCP server.
...
npm test -w @archon/sync-api                 # Fastify integration tests (need local Postgres)
...
3. **Integration tests hit real Postgres**, never mocks. The `integration-*.test.ts` files in `apps/archon-sync-api/src/` provision a throwaway schema and drop it in `finally`. Follow that pattern.
...
- Don't bypass Postgres through direct surgery when a REST or MCP path exists — add the path instead.
...
- **Tests fail with Postgres errors** — the test harness skips if `DATABASE_URL` (or `ARCHON_TEST_DATABASE_URL`) is unreachable. Make sure a local Postgres is up (`npm run docker:api:up`).
